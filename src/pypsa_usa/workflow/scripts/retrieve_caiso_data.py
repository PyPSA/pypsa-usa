...
def merge_fuel_regions_data(combined_data, year, fuel_regions_path):
    """Merge the fuel regions with the combined data."""
    df = pd.read_excel(fuel_regions_path, sheet_name="GPI_Fuel_Region")
...
def main(snakemake):
    ...
    combined_data_merged = merge_fuel_regions_data(
        combined_data,
        year=fuel_year,
        fuel_regions_path=snakemake.input.fuel_regions,
    )
