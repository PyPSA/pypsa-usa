...
Sector coupled models are built on top of the power sector representation. This means all assumptions, including spatial and temporal options, implemented to build the electricity sector are shared in sector coupled studies[^1].
...
:::{figure} _static/sectors/pypsa-sectors.png
:name: sector-res
:width: 600px
:alt: Sectoral interactions schematic
...
:::{figure} _static/sectors/pypsa-dr-soc.png
:name: dr-state-of-charge
:width: 700px
:alt: Demand response storage schematic

Schematic showing forwards and backwards demand response
:::

```{important}
Demand response in the transportation sector is applied at an aggregated vehicle level. This means all road vehicle types participate follow the same demand response characteristics (ie. demand response can not be split out so only light-duty vehicles participate). This is due to endogenous investments including both electric and gasoline vehicles.
```

### Capacity Constraints

Capacity constraints limit how much energy can be delivered through demand response at any point in time. To implement this, a custom constraint is implemented as shown below.
...
:::{figure} _static/dag_sector.jpg
:name: sector-workflow
:width: 700px
:alt: Sector workflow Snakemake DAG
...
- [Service Sector](./data-services.md)
