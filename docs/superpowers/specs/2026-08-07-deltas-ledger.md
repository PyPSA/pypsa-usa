...
| DL-17 | profile + all network stages (leap weather years) | Snapshot index: `_drop_leap_day` in `get_renewable_snapshots` strips Feb 29 so every GODEEEP CF axis is 8,760 h. | `fix_godeeep_time` shifts the raw GODEEEP axis from the leap day onward; without the drop the profile index and the network snapshots disagree. | Parity port: master's `_helpers.get_snapshots(drop_leap_day=True)` already yields 8,760 h, so the change is inert on the baseline (hot-fix ledger HF-22, ported to master-benchmark as `572534f`). Recorded here because HF-22 cites this id. | Kamran 2026-09-14 (via HF-22) |
| DL-18 | profile_{onwind,solar} (prong 2) and every downstream stage that reads `p_nom_max` | Master silently drops installable capacity: substations whose GODEEEP cells sum to zero availability, or whose polygon intersects no cell, are removed by `weighted_bus_aggregation` and `common_buses = profile ∩ caps ∩ regions` deletes their NREL caps. Develop retains it. Western: 7 onwind substations / 546 MW, 4 solar / 2,566 MW, incl. bus 37808 = the whole of cluster `p87 0` (develop-only cluster). | Master bug; develop sums caps to the cluster before aggregating. Predicted-vs-observed per-cluster delta matches to the cent (smoke `43585155`, 2026-09-15). | Waived on western prong 2 for `p_nom_max_by_zone_onwind` p8/p9/p10, `p_nom_max_by_zone_solar` p8 and both `system_potential_mw` findings, tagged HF-24. The companion finding from the same diagnosis is a CHANGE, not a fix of a master-parity gap: develop moved its cluster CF from a whole-cluster availability-weighted mean to availability-within-substation then `p_nom_max`-across-substations (`renewable.godeeep_cf_weighting: capacity`). That is not master's weighting either — master weights substation `p_max_pu` by EXISTING `p_nom` with a plain-mean fallback — so it is carried as its own row, DL-19 / HF-25, not as part of this one. | PENDING COUNTERSIGNATURE (Kamran) |
| DL-19 | clustered `p_max_pu` (`elec_s{simpl}`) and every network stage downstream of it — dispatch, `p_nom_opt`, objective | Cluster capacity factor: develop = availability-weighted over the GODEEEP cells of each substation, then `p_nom_max` (installable capacity) weighted across the cluster's substations, in one pass in `build_renewable_profiles`. Master = the same stage-1 substation mean, then `simplify_network` → pypsa 0.30.2 `clustering/spatial.py:aggregateoneport`, which weights by EXISTING `p_nom` and (`normed_or_uniform`) degenerates to a plain arithmetic mean over the cluster's substations when their `p_nom` sums to zero. Western: 12 of 20 onwind clusters have zero existing `p_nom`, so master's cluster CF there is the unweighted mean of its substations — a 1 MW polygon counts as much as a 10 GW one. | Master defect. Reconstructed from master's OWN outputs (western, `$SCRATCH/eq_verify/check_master_weights.py`, 2026-09-15): rebuilding `elec_s20` `p_max_pu` from the nodal `elec_base_network_l_pp.pkl`, existing-`p_nom`-weighting-with-uniform-fallback matches every cluster at \|Δ\| 0.00000, while `p_nom_max` weighting is off by mean \|Δ\| 0.0966 (onwind) / 0.0132 (solar). | ACCEPTED AS A DEVELOP IMPROVEMENT, not waived and not reverted (orchestrator decision, 2026-09-15): `p_nom_max` is the physically right weight for an extendable resource generator, whose buildable MW is what the profile is applied to; weighting by existing `p_nom` — and by nothing at all where no plant exists yet — makes a cluster's buildable resource quality depend on where capacity happens to stand today. Hot-fix HF-25. NOT observable at prong 2: the harness compares the profile files and rolls master's nodal file up with `p_nom_max` weights (`metrics.aggregate_profile_to_clusters`), the same weights develop uses, so `p_max_pu_quantiles_*` / `mean_cf_by_zone_*` are blind to it. It surfaces only once the clustered network is compared, which prong 2 does not yet do (backlog). Nothing waived against HF-25 for that reason. | PENDING COUNTERSIGNATURE (Kamran) |
| DL-20 | assembled network (`elec_s{simpl}_l_pp.pkl` / `elec_s{simpl}.nc`) and every stage downstream of it — clustered `p_nom`, `p_nom_opt`, dispatch, objective | Existing renewable capacity: master attaches LESS of the existing fleet than develop. Western (prong 2, assembled, 2026-09-15): onwind master 3,097.6 vs develop 6,554.5 MW (-3,456.9 MW, 52.7 % of develop's total, 32 of 61 substations); solar master 26,822.8 vs develop 31,188.0 MW (-4,365.2 MW, 14.0 %, 190 of 445). By ReEDS zone, develop - master: onwind p10 +121.6 %, p9 +99.7 %; solar p10 +18.9 %, p9 +12.7 %. | `workflow/scripts/add_electricity.py::attach_renewable_capacities_to_atlite` is IDENTICAL on both branches; only the bus space it runs on differs. Its last step is `mapped_values = generators_tech.sub_assignment.map(caps_per_bus).dropna()`, which can only land a plant's MW on a bus that already carries a profile-derived generator of that carrier. Master runs it on the NODAL network grouped by `sub_id`, and master's western profile files cover only 544 of 1,972 onwind and 808 of 1,972 solar substations, so every plant at an uncovered substation is dropped — master's own log prints it ("N GW of <tech> plants that are not in the network. See git issue #16"). Develop runs the same function after simplify-early, at s{simpl} CLUSTER granularity, where a cluster carries a profile generator if ANY of its substations does, so nothing is lost. Same mechanism as DL-9. | Master data-quality bug, fixed on develop by construction. Expected downstream effect, to be confirmed on the solved leg: develop starts with more existing renewable capacity, so LOWER objective, LOWER extendable solar/onwind `p_nom_opt`, more renewable and less gas dispatch. Waived on western prong 2 for `capacity_existing_by_carrier` onwind (+, <=120 %) and solar (+, <=20 %), and `p_nom_existing_by_zone_carrier` `p10 \| onwind` / `p9 \| onwind` (+, <=130 %) and `p10 \| solar` / `p9 \| solar` (+, <=20 %), tagged HF-26. Figures: `experiments/runs/eq-western-db47198d8592/diagnosis/fig6_by_reeds_zone.png`, `fig5_mw_accounting.png`. | PENDING COUNTERSIGNATURE (Kamran) |
| DL-21 | assembled network (`elec_s{simpl}_l_pp.pkl` / `elec_s{simpl}.nc`), per ReEDS zone only | **NOT A DELTA — a develop bug, now fixed.** Geographic assignment of existing capacity: a plant sat in a different ReEDS zone on the two sides. Net zero system-wide, by every carrier. Western (prong 2, assembled, 2026-09-15): 101.2 MW of onwind moved p9 -> p8 (master p8 onwind 0.0 MW, develop 101.2 MW) and ~166.6 MW of solar left p11 (189.4 -> 73.3 MW, -61.3 %). The same relocation moved conventional and storage capacity: PHS 1,053 MW p9 -> p10, geothermal 84.4 MW p10 -> p9, biomass 97.0 MW p9 -> p8, and tens of MW each of CCGT, OCGT, battery and oil. 33 plants / 1,320 MW in all, 6.1 % of existing MW. | The earlier entry said this was a master defect. It was not. `match_plant_to_bus` is the SAME code on both branches, and its zone-constrained first pass is inert on BOTH (it keys `plants["country"]`, county codes like `p06029`, against `buses.reeds_zone`, `p10`), so both fall through to the unconstrained nearest-point match. What differs is the bus set that match runs against: master runs on the NODAL network (4,248 western buses, one per substation) — median plant -> bus 1.0 km, 100.00 % of existing MW inside the plant's own ReEDS polygon by lat/lon ground truth — while develop, after simplify-early, ran it against the 20 `{simpl}` cluster CENTROIDS: median 37 km, 93.89 % in the right zone, and master right for 32 of the 33 misplaced plants. USA is worse by construction (s300 over 134 zones, 2.24 clusters per zone) and `clusters: 134` (one bus per zone) makes it permanent. | FIX on develop, not a waiver: `add_electricity.match_plant_to_bus` now takes `bus2sub.csv` (all rows — exactly master's bus set) and `busmap_s{simpl}.csv`, matches each plant to its nearest SUBSTATION with the same raw-degree Euclidean metric, and maps that substation to its cluster bus. The reconstruction reproduces master's zone for 841 of 841 western plants (100.00 % of MW) and leaves 0 of the 11 non-equivalent `p_nom_existing_by_zone_carrier` rows. The three HF-27 waivers (`p11 \| solar`, `p8 \| onwind`, `p8 \| solar`) were REMOVED with the fix and HF-27 is now `usa_noop: true`, so a zone row that differs again comes back UNEXPLAINED instead of inheriting this signature. The conventional and storage zone moves were never waived either. Figures: `experiments/runs/eq-western-db47198d8592/diagnosis/fig8..fig11*`, `fig6_by_reeds_zone.png`, `fig5_mw_accounting.png`. | PENDING COUNTERSIGNATURE (Kamran) |
| DL-22 | base network `reeds_zone` labels, hence `demand_by_zone` and `p_nom_existing_by_zone_carrier` per ReEDS zone (usa prong 2); system totals conserved | Zone relabel: 101 of 82,549 nodal buses carry a different `reeds_zone` on master. 17 downtown-DC buses move 5,767.6 MW of mean demand p99 -> p123 (Virginia -34 %, Maryland+DC +27 %); the plants on the relabelled buses move with them: 37 units / 2,071.5 MW of existing capacity in exact ± pairs (Marcus Hook CCGT 836.1 MW p126 -> p122, Bayonne OCGT 644.0 MW p127 -> p126, Rio Grande OCGT 298.3 MW p59 -> p31, Fish Springs solar 100 MW + battery 25 MW p12 -> p8, and smaller DC / Delaware-Valley / Chesapeake moves). | Master defect (HF-5): `build_base_network.assign_missing_regions` copies ALL seven region attributes from the nearest complete bus whenever any one is missing, clobbering the correct `county`; `assign_reeds_memberships` then takes the modal zone per county. Develop fills only the NaN attributes. develop's `county` matches the Census polygon on 101 / 101 relabelled buses, master's on 0 / 101. Reconstruction: relabelling develop's per-plant table with master's zone for the same substation reproduces master's column to abs(residual) <= 3e-13 MW on all 503 (zone, carrier) rows; the demand reconstruction to 0.048 MW on 134 zones. | DECIDED 2026-09-17 (Kamran): explained, develop correct, no fix. Bounded `max_abs_delta_mw` table waivers on usa prong 2 tagged HF-5: 16 conventional/storage rows, 6 solar rows (whole delta, HF-26 dropped + HF-5 moved) and `demand_by_zone` p99/p123 mean and peak. Figures: `experiments/runs/eq-usa-80abb3bc125f/diagnosis/fig1..fig4*`, `experiments/runs/eq-usa-82c25a3177d0/diagnosis/fig6_zone_relabel_mw_moved.png`, `fig7_relabelled_buses_map.png`. | Kamran 2026-09-17 |
