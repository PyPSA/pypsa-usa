...
| DL-21 | assembled network (`elec_s{simpl}_l_pp.pkl` / `elec_s{simpl}.nc`), per ReEDS zone only | Geographic assignment of existing capacity: a plant may sit in a different ReEDS zone on the two sides. Net zero system-wide, by every carrier. Western (prong 2, assembled, 2026-09-15): 101.2 MW of onwind moves p9 -> p8 (master p8 onwind 0.0 MW, develop 101.2 MW) and ~166.6 MW of solar leaves p11 (189.4 -> 73.3 MW, -61.3 %). The same relocation moves conventional and storage capacity: PHS 1,053 MW p9 -> p10, geothermal 84.4 MW p10 -> p9, biomass 97.0 MW p9 -> p8, and tens of MW each of CCGT, OCGT, battery and oil. | Master's zone-constrained first-pass plant -> bus match is INERT: it keys `plants["country"]` (values like `p06023`) against `buses.reeds_zone` (values like `p9`), two vocabularies that never intersect, so every plant falls through to the unconstrained nearest-bus fallback and may cross a zone boundary. Develop assigns each plant to the cluster whose REGION contains it, so a plant cannot leave the zone it stands in. Read from the code and measured per zone; not yet tied together plant by plant, hence HF-27 confidence `medium`. | Master bug; develop's assignment is the correct one. Waived on western prong 2 for `p_nom_existing_by_zone_carrier` `p11 \| solar` (-, <=65 %), `p8 \| onwind` and `p8 \| solar` (absolute bound, <=200 MW — master carries 0.0 / 10.1 MW there, so \|delta %\| is undefined or noise), tagged HF-27. The conventional and storage zone moves listed above are deliberately NOT waived: they were not part of the signed-off measurement, and leaving them UNEXPLAINED is what forces them to be judged on their own evidence. Figures: `experiments/runs/eq-western-db47198d8592/diagnosis/fig6_by_reeds_zone.png`, `fig5_mw_accounting.png`. | PENDING COUNTERSIGNATURE (Kamran) |
