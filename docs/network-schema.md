...
|--------|-------|----------|-------------|-------------|------------|-------------|
| LAF_state | float | build_base_network.py | aggregate_to_substations.py, cluster_network.py, add_electricity.py, build_demand.py | sum | NaN = bus has no Pd / offshore (filled with 0 in build_demand) | Load Allocation Factor within a state; `Pd / sum(Pd) over full_state`. Not currently in `aggregate_to_substations.bus_strategies` — the recurring `consense` crash for this column is the bug class this catalog was created to address. |
