...
def compare_structures(data1, data2, path="", yaml_name=""):
    ...
    if isinstance(data1, dict):
        for key in data1:
            if key not in data2:
                print(f"Missing key '{key}' in second structure at {path} in {yaml_name}")
                continue
            compare_structures(
                data1[key],
                data2[key],
        ...
        for key in data2:
            if key not in data1:
                print(f"Missing key '{key}' in first structure at {path} in {yaml_name}")
        return True
    elif isinstance(data1, list):
        # For simplicity, just compare the first item if it exists, assuming homogeneous lists
        if data1 and data2:
            compare_structures(data1[0], data2[0], path=f"{path}[0]", yaml_name=yaml_name)
