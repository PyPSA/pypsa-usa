...
snakemake -j6 --cluster "sbatch --time={resources.time_min} --cpus-per-task=1 --mem={resources.mem_mb} --partition=serc --account=iazevedo -o logs/slurm/{rule}_{wildcards} -e logs/slurm/{rule}_{wildcards}" --latency-wait 60