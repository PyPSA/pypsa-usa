...
# snakemake -j6 --cluster "sbatch --time={resources.time_min} --partition=serc --account=iazevedo -c {resources.cpus} -o logs/slurm/{rule}_{wildcards} -e logs/slurm/{rule}_{wildcards}"