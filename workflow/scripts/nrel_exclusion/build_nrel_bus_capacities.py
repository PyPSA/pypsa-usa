...
def filter_supply_curve_points(
    ...
    if apply_cec and tech in ("onwind", "solar"):
        ...
        if in_ca.any():
            ...
            print(f"[caps] CEC filter ({tech}): dropped {n_drop}/{in_ca.sum()} "
                  f"California-internal sites")
        else:
    ...
    if apply_boem and tech.startswith("offwind"):
        ...
        keep = keep & boem_keep
        print(f"[caps] BOEM filter ({tech}): dropped {n_drop}/{len(df)} "
              f"sites outside planning areas")
...
def rollup_supply_curve(
    ...
    if tech in OFFSHORE_TECH_FILTER:
        ...
        if "technology" not in df.columns:
            raise ValueError(
                f"supply curve {csv_path} missing `technology` column needed "
                f"to split fixed vs floating offshore sites"
            )
        n_before = len(df)
    ...
    sub = df[["latitude", "longitude", "capacity_ac_mw", cf_col]].rename(
        columns={cf_col: "cf"}
    ).copy()
    ...
    gdf_shapes = (
        pd.concat(shapes, axis=0, ignore_index=True)[["name", "geometry"]]
    ...
    if joined.empty:
        print(f"[caps] no surviving sites — writing empty caps Dataset")
    ...
    bus_centroids = (
        gdf_shapes.set_index("name").to_crs("EPSG:3857").geometry.centroid.to_crs(4326)
    )
    ...
    joined["dist_km"] = haversine_km(
        joined["latitude"].values, joined["longitude"].values,
        joined["bus_lat"].values, joined["bus_lon"].values,
    )
    ...
    def agg(group: pd.DataFrame) -> pd.Series:
        ...
        if cap_sum <= 0:
            return pd.Series(
                {"p_nom_max": 0.0, "weight": 0.0, "avg_cf": np.nan,
                 "average_distance": np.nan, "site_lat": np.nan, "site_lon": np.nan}
            )
    ...
    print(f"[caps] rolled up to {len(rollup)} buses; "
    ...
    if offshore_eez_shape is not None:
        ...
        print(f"[caps] underwater_fraction: "
              f"mean={np.nanmean(uf):.3f}  min={np.nanmin(uf):.3f}  max={np.nanmax(uf):.3f}")
...
def main():
    ...
    ap.add_argument("--offshore-eez-shape", default=None,
