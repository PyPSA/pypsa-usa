	...
	for i in range(len(regions_onshore)):
		...
		if logger is not None:
			if co2_storage_potential == 0:
				...
				logger.info("Node '%s' has an aggregated CO2 storage potential of %0.1f MtCO2 with an average cost of %0.2f USD/tCO2" % (node_name, co2_storage_potential, co2_storage_cost_average))
...
if __name__ == "__main__":
	...
	if "snakemake" in globals():
		build_co2_storage(snakemake.input["regions_onshore"], snakemake.input["co2_storage"], snakemake.output["co2_storage"], logger)
	else:
