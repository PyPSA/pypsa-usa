'''
Script takes a given pypsa network and creates stochastic profiles for load wind and solar.
'''
#%% Imports and functions
import numpy as np, pandas as pd, os, sys, time, glob, pandas as pd, numpy as np, matplotlib.pyplot as plt, os, xarray as xr
...
def import_profiles_from_network(PATH_NETWORK, num_clusters):
    ...
    network.loads_t.p_set.to_csv(os.path.join('resources/pypsa', f'loads_t_p_set_{num_clusters}.csv'))

    #filter columns with solar in name
    solar_df = network.generators_t.p_max_pu.loc[:,network.generators_t.p_max_pu.columns.str.contains('solar')]
    solar_df.to_csv(os.path.join('resources/pypsa', f'solar_profiles_{num_clusters}.csv'))

    #filter columns with wind in name and export as csv
    wind_df = network.generators_t.p_max_pu.loc[:,network.generators_t.p_max_pu.columns.str.contains('wind')]
...
def resample_and_group(base_data, area_mapping):
    ...
    """
    base_area = base_data.groupby(base_data.columns.map(area_mapping.area_id), axis=1).mean()
...
def create_allocation(base, mapping):
    ...
    total_area_hourly = base_.groupby(base_.columns.map(mapping.to_dict()['area_id']), axis=1).sum()
...
def assign_stochastic_mu(df_params, timestamp_reference, column_name= 'profile_type'):
    '''
    Assigns a stochastic mu to each hour of the year based on the season and profile type.
    ...
    '''
    df_mu_hourly = pd.merge(left=timestamp_reference, right=df_params[[column_name,'area_id','season','mu']], left_on='season_num', right_on='season', how='outer')
...
def sampling_warmup(df, thresholds, rng, min_steps=50):
    ...
    for _ in range(min_steps):
        ...
        ratio, eps, na = resample_ratio(ratio, winter_params, thresholds, eps, rng, True)
    return ratio, eps
...
def resample_ratio(original_ratio, params, thresholds, original_epsilon, rng, initialization):
    resample_count = 0
    param_dict = params.sampling_id.to_dict()
    while np.any(np.logical_or(original_ratio > thresholds, original_ratio < 0)): #if any values are out of bounds
        resampled_ratio = original_ratio.copy(deep=True)
        mask_to_resample = np.logical_or(original_ratio > thresholds, original_ratio < 0) #mask of values to resample
        resampled_epsilon = rng_eps(params[mask_to_resample.values], rng) #resample epsilon for values to resample
        resampled_ratio[mask_to_resample] = original_ratio[mask_to_resample] - original_epsilon[mask_to_resample] + resampled_epsilon.values #resample ratio. subtract original epsilon and add new epsilon
        resample_count += 1

        if np.any(np.logical_or(resampled_ratio > thresholds, resampled_ratio < 0)): #if any values are still out of bounds

            in_bounds_mask = np.logical_and(mask_to_resample, np.logical_and(resampled_ratio < thresholds, resampled_ratio > 0))
            

            if np.any(in_bounds_mask): #if any values are in bounds (i.e. resampling was successful), then update original ratio and epsilon so that we reduce the number of resampled values in the next iteration
...
if __name__ == "__main__":
    ...
    num_clusters = snakemake.wildcards.clusters

    ################### Sampling ###################
    #%%
    load_base, solar_base, wind_base = import_profiles_from_network(NETWORK_PATH, num_clusters)
    num_zones = {'load': load_base.shape[1], 'solar': len(area_mapping_solar.area_id.unique()), 'wind': len(area_mapping_wind.area_id.unique())}
    ...
    #Define Sampling IDs
    load_parameters['sampling_id'] = load_parameters.index + '_' + load_parameters.timescale + '_' + load_parameters.area_id.astype(str)
    solar_parameters['sampling_id'] = solar_parameters.index + '_' + solar_parameters.timescale + '_' + solar_parameters.area_id.astype(str)
    wind_parameters['sampling_id'] = wind_parameters.index + '_' + wind_parameters.timescale + '_' + wind_parameters.area_id.astype(str)
    parameters_concat = pd.concat([load_parameters, solar_parameters, wind_parameters]).reset_index() 

    #Create a timeseries index dataframe to reference when sampling
    timestamp_reference = pd.DataFrame({'timestamp': pd.period_range(year, periods=num_hours, freq='h')})
    timestamp_reference = timestamp_reference.assign(month=timestamp_reference['timestamp'].dt.month, day=timestamp_reference['timestamp'].dt.day, hour=timestamp_reference['timestamp'].dt.hour, day_of_year=timestamp_reference['timestamp'].dt.dayofyear)
    timestamp_reference['season'] = timestamp_reference['month'].map({12: 'winter', 1: 'winter', 2: 'winter', 3: 'spring', 4: 'spring', 5: 'spring', 6: 'summer', 7: 'summer', 8: 'summer', 9: 'fall', 10: 'fall', 11: 'fall'})
    timestamp_reference['season_num'] = timestamp_reference['season'].map({'winter': 1, 'spring': 2, 'summer': 3, 'fall': 4})
    ...
    solar_base_area_daily, solar_base_area = resample_and_group(solar_base, area_mapping_solar)
    ...
    df_daytime_solar_tracking = pd.DataFrame(np.zeros_like(solar_base_area), index=solar_base_area.index, columns=solar_base_area.columns)
    df_daytime_solar_tracking = df_daytime_solar_tracking.apply(lambda x: define_solar_hours(daytime_mask[x.name], timestamp_reference)[2])

    #%%#Define Stochastic Mu Dataframes
    cols_to_use = timestamp_reference.columns.difference(load_parameters.columns)
    df_mu_load_hourly_unstack = assign_stochastic_mu(load_parameters.reset_index(), timestamp_reference[cols_to_use])
    ...
    df_mu_solar_daily_unstack = assign_stochastic_mu(solar_parameters.query(" timescale == 'daily' ").reset_index(), timestamp_reference)
    df_mu_wind_hourly_unstack = assign_stochastic_mu(wind_parameters.query(" timescale == 'daily' ").reset_index(), timestamp_reference[cols_to_use])
    ...
    ratio_samples_combined = np.zeros([num_hours, parameters_concat.sampling_id.unique().shape[0], num_samples])
    ...
    for sample_num in range(0,num_samples):    
        ...
        solar_sampling_id_dict = dict(parameters_concat[parameters_concat.sampling_id.str.contains('solar')].reset_index()[['sampling_id', 'index']].values)
        ratio_samples = pd.DataFrame(0, index=np.arange(n_hours), columns=sampling_profile_names)
        epsilon_samples = pd.DataFrame(0, index=np.arange(n_hours), columns=sampling_profile_names)
        ...
        ratio_samples.iloc[0, :], epsilon_samples.iloc[0, :] = sampling_warmup(parameters_concat,thresholds, rng)
        ...
        for i in range(1,n_hours):
            ...
            ratio_samples.iloc[i, :] = ratio_samples.iloc[i - 1, :] + season_df.kai.values * (season_df.mu.values - ratio_samples.iloc[i - 1, :]) + p_eps.values
            epsilon_samples.iloc[i, :] = p_eps.values

            if i % 24 != 0:
                ratio_samples.loc[i, ratio_samples.columns.str.contains('daily')] = ratio_samples.loc[i - 1, ratio_samples.columns.str.contains('daily')]
                epsilon_samples.loc[i, epsilon_samples.columns.str.contains('daily')] = epsilon_samples.loc[i - 1, epsilon_samples.columns.str.contains('daily')]

            previous_sunset_idx = df_daytime_solar_tracking.iloc[i].astype(int)
            if (previous_sunset_idx > 0).any():
                area_sunset_dict = dict(zip([solar_sampling_id_dict.get(param_code) for param_code in ratio_samples.columns if 'solar' in param_code], previous_sunset_idx[previous_sunset_idx > 0].index))
                previous_mu_vals = ratio_samples.loc[:,ratio_samples.columns.str.contains('solar')].iloc[i - 1]
                ...
                solar_mu_vals, solar_kai_vals = solar_season[['mu', 'kai']][solar_season.sampling_id.isin(previous_mu_vals.index)].values.T
                eps_filtered = p_eps[solar_season.index].rename(index=season_df.sampling_id.to_dict()).loc[previous_mu_vals.index].values
                ratio_samples.loc[i, previous_mu_vals.index] = previous_mu_vals.values + solar_kai_vals * (solar_mu_vals - previous_mu_vals.values) + eps_filtered

            if (ratio_samples.iloc[i] > thresholds).any() or (ratio_samples.iloc[i] < 0).any():
                r0, eps = ratio_samples.iloc[i], epsilon_samples.iloc[i, :]
                ratios_new, eps_new, resample_count = resample_ratio(r0, season_df, thresholds, eps, rng, False)
                ratio_samples.iloc[i], epsilon_samples.iloc[i, :], resample_count = ratios_new, eps_new, resample_count
        ...
        sampled_solar = solar_base_area.values * (0.333 * sampled_hourly_solar_ratio.values + 0.667 * sampled_daily_solar_ratio.values) / (df_mu_solar_hourly_unstack.iloc[:,1:] * 0.333 + df_mu_solar_daily_unstack.iloc[:,1:] * 0.667)
        ...
        sampled_solar_generatorlvl = pd.DataFrame(sampled_solar_generatorlvl, index=sampled_solar.index, columns= solar_allocation.columns)
        sampled_solar_generatorlvl = sampled_solar_generatorlvl.apply(lambda x: x * sampled_solar.loc[:,str(x.name)], axis=0)
        ...
        sampled_wind = wind_base_area.values * (0.333 * sampled_hourly_wind_ratio.values + 0.667 * sampled_daily_wind_ratio.values) / (df_mu_wind_hourly_unstack.iloc[:,1:] * 0.333 + df_mu_wind_daily_unstack.iloc[:,1:] * 0.667)
        ...
        sampled_wind_generatorlvl = pd.DataFrame(sampled_wind_generatorlvl, index=sampled_wind.index, columns= wind_allocation.columns)
        sampled_wind_generatorlvl = sampled_wind_generatorlvl.apply(lambda x: x * sampled_wind.loc[:,str(x.name)], axis=0)
    ...
    df_ind_match = df_ind_match.merge(solar_cols, left_on='load', right_index=True, how='outer').merge(wind_cols, left_on='load', right_index=True, how='outer')
    ...
    df_ind_match_combinedarr = df_ind_match.where(df_ind_match.isnull(), np.column_stack([df_ind_match.index.values]*3))

    # Create index references for each dataset
    solar_inds_combined = df_ind_match_combinedarr.np_ind_solar.values[~np.isnan(df_ind_match_combinedarr.np_ind_solar.values)].astype(int)
    wind_inds_combined = df_ind_match_combinedarr.np_ind_wind.values[~np.isnan(df_ind_match_combinedarr.np_ind_wind.values)].astype(int)
    load_inds_combined = df_ind_match_combinedarr.np_ind_load.values[~np.isnan(df_ind_match_combinedarr.np_ind_load.values)].astype(int)
    ...
    combined_gen_samples = np.zeros([8784, 97, 3, num_samples]) # Dimensions are (t, node, profile type, samples)
    combined_gen_samples[:,load_inds_combined,0,:] = load_zones_samples[:,load_inds_orig,:]
    combined_gen_samples[:,solar_inds_combined,1,:] = solar_gen_samples[:,solar_inds_orig,:]
    combined_gen_samples[:,wind_inds_combined,2,:] = wind_gen_samples[:,wind_inds_orig,:]

    # Create xarray dataset
    da_bus_data = xr.DataArray(combined_gen_samples,
                    coords={'timestamp': timestamp_reference.timestamp.dt.to_timestamp() , 'bus': df_bus_coordinates.values, 'profile_type': ['load','solar','wind'], 'sample_num': np.arange(0,num_samples)}, 
    ...
    base_zones_solar = pd.DataFrame(solar_cols.index.str.replace('[0-9]','').drop_duplicates(), columns=['solar'])
    base_zones_wind = pd.DataFrame(wind_cols.index.str.replace('[0-9]','').drop_duplicates(), columns=['wind'])

    base_zones_wind = base_zones_wind[~base_zones_wind.wind.str.contains('SDGE')].reset_index(drop=True) #temporary fix
    df_zone_ind_match = base_zones_solar.reset_index(names='solar_ind').merge(base_zones_wind.reset_index(names='wind_ind'), left_on='solar',right_on='wind', how='outer')
    ...
    combined_zone_data = np.zeros([8784, 22, 2, num_samples]) # Dimensions are (t, node, profile type, samples)
    combined_zone_data[:,solar_zone_inds_combined,0,:] = solar_zones_samples[:,solar_zone_inds_orig,:]
    combined_zone_data[:,wind_zone_inds_combined,1,:] = wind_zones_samples[:,wind_zone_inds_orig,:]

    da_zone_data = xr.DataArray(combined_zone_data,
                    coords={'timestamp': timestamp_reference.timestamp.dt.to_timestamp() , 'zone': zone_coords.values, 'profile_type': ['solar','wind'], 'sample_num': np.arange(0,num_samples)}, 
                    dims=['timestamp', 'zone', 'profile_type', 'sample_num'])

    da_ratios = xr.DataArray(ratio_samples_combined,
                            coords={'timestamp':  timestamp_reference.timestamp.dt.to_timestamp() , 
    ...
    np.save(os.path.join(os.getcwd(),PATH_SAMPLE,'combined_gen_samples.npy'), combined_gen_samples)
    np.save(os.path.join(os.getcwd(),PATH_SAMPLE,'combined_ratio_samples.npy'), ratio_samples_combined)