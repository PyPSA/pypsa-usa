...
from build_powerplants import load_pudl_data, filter_outliers_iqr_grouped, filter_outliers_zscore
...
def build_pudl_fuel_costs(snapshots: pd.DatetimeIndex, start_date: str, end_date: str):
    ...
    fuel_cost_temporal['interconnect'] = fuel_cost_temporal['nerc_region'].map(const.NERC_REGION_MAPPER)
    ...
    fuel_cost_temporal['generator_name'] = (
        fuel_cost_temporal['plant_name_eia'].astype(str) + '_' + 
        fuel_cost_temporal['plant_id_eia'].astype(str) + '_' + 
