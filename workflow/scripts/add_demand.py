...
Depending on study, the load will all be aggregated to a single load type, 
or distributed to different sectors and end use fuels.
"""
...
def attach_demand(n: pypsa.Network, df: pd.DataFrame, load_name: str, carrier: str):
    ...
    buses = df.columns
    df = df.rename({
        x:f"{x}{load_name}" for x in buses
    })
    n.madd(
        "Load",
        df.columns,
