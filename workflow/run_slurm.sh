...
# 134 node network, egs_conus_z134_nz2050 Started June 9 2026 (myopic-fixes branch)
snakemake --cluster "sbatch -A {cluster.account} --mail-type ALL --mail-user {cluster.email} -p {cluster.partition} -o {cluster.output} -e {cluster.error} -c {threads} --mem {resources.mem_mb} --time {cluster.walltime}" \
    --cluster-config config/260609_config/config.cluster.yaml \
    --jobs 40 --latency-wait 60 --rerun-incomplete \
    --configfile config/260609_config/config.egs_conus_z134_nz2050_260609.yaml \
    --set-resources add_electricity:mem_mb=200000

# 134 node network, egs_conus_z134_nz2050 Started April 26 2026 (Includes quality 1 2)
# snakemake --cluster "sbatch -A {cluster.account} --mail-type ALL --mail-user {cluster.email} -p {cluster.partition} -o {cluster.output} -e {cluster.error} -c {threads} --mem {resources.mem_mb} --time {cluster.walltime}" \
#     --cluster-config config/260310_config/config.cluster.yaml \
#     --jobs 40 --latency-wait 60 --rerun-incomplete \
#     --configfile config/260310_config/config.egs_conus_z134_nz2050_q12.yaml \
#     --set-resources add_electricity:mem_mb=200000
